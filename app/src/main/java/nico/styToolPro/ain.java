...
public class ain implements IXposedHookLoadPackage {
    ...
    public void handleLoadPackage(final LoadPackageParam lpparam) throws Throwable {
        if (lpparam.packageName.equals(WECHAT_PACKAGE_NAME)) {
                    ...
                    if (activity != null) {
                        ...
                        if (intent != null) {
                            ...
                            if (!TextUtils.isEmpty(className) && className.equals("com.tencent.mm.ui.LauncherUI") && intent.hasExtra("donate")) {
                                ...
                                donateIntent.putExtra("pay_scene", 132);
            ...
            findAndHookMethod(ersionParam.getMessageClass, lpparam.classLoader, "b", Cursor.class, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (type == 431207665 || type == 469762097) {
                        ...
                        String notContainsWords = PreferenceUtils.notContains();
                        ...
                        int delayTime = 0;
                        if (PreferenceUtils.delay()) {
                            delayTime = getRandom(PreferenceUtils.delayMin(), PreferenceUtils.delayMax());
                        }
                        callMethod(callStaticMethod(findClass(ersionParam.GET_LUCKY_MONEY_CLASS, lpparam.classLoader), ersionParam.getNetworkByModelMethod), "a", ab, delayTime);
                    }
                }
            });


            findAndHookMethod(LUCKY_MONEY_RECEIVE_UI_CLASS_NAME, lpparam.classLoader, ersionParam.receiveUIFunctionName, int.class, int.class, String.class, ersionParam.receiveUIParamName, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    if (PreferenceUtils.quickOpen()) {
            ...
            findAndHookMethod("com.tencent.mm.plugin.profile.ui.ContactInfoUI", lpparam.classLoader, "onCreate", Bundle.class, new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    if (PreferenceUtils.showWechatId()) {
                        Activity activity = (Activity) param.thisObject;
